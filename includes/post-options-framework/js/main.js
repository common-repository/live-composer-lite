jQuery(document).ready(function(){
	...
	if ( jQuery('#postcustom input[value="dslc_code"]').val() == 'dslc_code' ) {
		jQuery('#wp-content-wrap').removeClass('html-active');
		jQuery('#postdivrich').addClass('lc-active');
	} else {
		jQuery('#lc_content_wrap h2').text('Сreate your page in Live Composer');
	}
...
(function($) {
  ...
  $(document).on('click', '#content-lc', function(e) {
    e.preventDefault();
    $('.wp-editor-expand').addClass('lc-active');
  });

  $(document).on('click', '#content-tmce, #content-html', function(e) {
    e.preventDefault();
    $('.wp-editor-expand').removeClass('lc-active');
  });
