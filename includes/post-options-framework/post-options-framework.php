...
function dslc_post_add_row_action( $actions, $post ) {
	...
	if ( $post_status != 'trash' && $post_type != 'post' ) {
		if ( array_key_exists( $post_type, $dslc_var_templates_pt ) ) {
			$template_id = dslc_st_get_template_ID( $post->ID );
			$actions = array('edit-in-live-composer' => '<a href="'. get_home_url() . '/?page_id=' . $template_id . '&dslc=active">'. __( 'Edit in Live Composer', 'live-composer-page-builder' ) .'</a>') + $actions;
		} else {
...
function dslc_tab_content( $content ) { ?>
	<div id="lc_content_wrap">
	    <h2> <?php _e( 'Your page created in Live Composer', 'live-composer-page-builder' ); ?></h2>
	    <div class="description"><?php _e( 'All the content stored as custom field', 'live-composer-page-builder' ); ?></div>
